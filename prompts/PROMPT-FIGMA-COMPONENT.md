...
1. Utilise `get_design_context` puis `get_variable_defs` sur l'URL fournie pour analyser la structure et les variables Figma. Lis les collections de variables dans cet ordre : `Fondations` → `Typographie` → `Theme-color` → `Effets`. Ignore les fills, styles locaux et couleurs directement appliquées sur les calques.
...
Une fois ton analyse terminée, présente-moi un rapport structuré :
- Mapping tokens Figma → CSS (variables résolues et manquantes)
- Icônes : présentes ✓ / manquantes (bloquer si manquantes)
- Décision animation : CSS transition ou Framer Motion (justifier)
- Dossier cible : `ui/` ou `blocks/` (justifier)

⚠️ Attends mon "GO" avant d'écrire le moindre code.

---

Après mon "GO", génère le composant avec ces contraintes absolues :
...
- Classes CSS : utilise toujours `cn` (de `@/lib/utils`) pour composer les classes — jamais de `.filter(Boolean).join(" ")`.
...
1. Use `get_design_context` then `get_variable_defs` on the provided URL to analyse the component structure and Figma variables. Read variable collections in this order: `Fondations` → `Typographie` → `Theme-color` → `Effets`. Ignore fills, local styles, and colours applied directly on layers.
...
Once your analysis is complete, present a structured report:
- Token mapping Figma → CSS (resolved variables and missing ones)
- Icons: present ✓ / missing (block if missing)
- Animation decision: CSS transition or Framer Motion (justify)
- Target folder: `ui/` or `blocks/` (justify)

⚠️ Wait for my "GO" before writing any code.

---

After my "GO", generate the component with these absolute constraints:
...
- CSS classes: always use `cn` (from `@/lib/utils`) to compose classes — never `.filter(Boolean).join(" ")`.
